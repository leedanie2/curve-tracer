...
| Base resistor | **330 Ω** | Set by op-amp output current, not damping. Limiter trip: ~13.95 V / `R_B` = **42 mA**, under the OPA2197's ~65 mA *typical* short-circuit current (220 Ω → 63 mA leaves no margin). Clamp-diode discharge: **21.9 mA** peak sink (sim 09). Overshoot into 100 nF is 10.3% small-signal / 2.2% full-scale — well damped (sim 08). See below |
...
**Base resistor sizing — why 100 Ω is wrong.** When the current limiter trips, the op-amp output rails (~14.8 V) and drives the limiter transistor's collector — the BD139 base, held near 0.85 V — through `R_B`. The op-amp must then supply about 13.95 V / `R_B`: **140 mA at 100 Ω**, which exceeds the OPA2197's own short-circuit current of about **65 mA**. The op-amp's internal protection would engage first, so the limit test would be measuring *the op-amp's* protection rather than the circuit's — a meaningless result. **330 Ω** brings the demand to **42 mA**, so the external limiter is what actually sets the threshold. 220 Ω (63 mA) sits at the typical limit with no margin.

**The 65 mA figure is typical, not guaranteed.** The OPA2197's short-circuit current varies with output voltage and temperature, so 42 mA is margin against a typical value, not a worst case. **The limiter-trip case is not simulated:** the op-amp model in sims 08/09 (LTspice `UniversalOpAmp2`, default parameters) clamps its output at 25 mA — below every current in question — so it cannot show where the real part's own limit engages. **Bench verification required:** with the limiter tripped into a short, measure the op-amp output current (the drop across `R_B`) and confirm the op-amp is not in its own current limit.
...
- **Clamp diode discharge (stage 7).** Sim 09 has the op-amp sinking **21.9 mA** through the clamp on every falling edge (100 nF, open load). The LM324 sinks only **10 mA min / 20 mA typ**. It also has a weak-sink region near ground: its output cannot pull below roughly 0.5 V while sinking more than ~50 µA, because the internal 100 µA source current dominates. So the clamp path stops pulling the emitter at about 0.5 V + V_f ≈ 1.1 V. Fast discharge toward 0 V is exactly what the clamp exists to do, so on an LM324 the falling edge will read slow and floor early. That is the op-amp, not the circuit. At 100 pF the sink is only 1 mA, but the LM324's slew rate (roughly 0.5 V/µs, against 10 V/µs in the sim model) limits the fall instead. The stage 7 clamp measurement, and any comparison against sim 09's 1.7 µs, require the OPA2197.
- **Short test (stage 8).** The LM324's own output source current is **20 mA min, 40 mA typ**. The minimum is what matters, and it is well under the ~42 mA that `R_B` = 330 Ω asks for when the limiter trips. The op-amp, not the external limiter, will set the current — the meaningless result §3.1 warns about. Treat a stage 8 short test on the LM324 as functional only; repeat it on the OPA2197.
